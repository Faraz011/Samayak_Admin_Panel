...
export default function PdfIngestionPage() {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      await supabase.from('pdf_ingestions').insert({
        file_path: filePath,
        uploaded_by: user?.id,
      ...
      await fetch('/api/pdf-ingestions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ file_path: filePath, department_id: selectedDept }),
      });
            ...
            >
              ...
              {uploading ? (
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="font-bold text-ink">Uploading and queuing…</p>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-3">
