...
import { parseTimetablePdf } from '@/lib/pdf-processor';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (ingestion.status === 'parsing' || ingestion.status === 'integrating') {
      return NextResponse.json({ error: 'Ingestion is already being processed' }, { status: 400 });
    }
    ...
    // 4. Retrieve PDF base64 content
    const base64Content = ingestion.file_content;
    if (!base64Content) {
      throw new Error('No PDF content found in database row');
    }

    // 5. Parse PDF using processor
    console.log(`[Ingestion ${ingestionId}] Starting PDF parse...`);
    const parseResult = await parseTimetablePdf(base64Content);
    console.log(`[Ingestion ${ingestionId}] Parsed ${parseResult.rows.length} rows using ${parseResult.method}`);

    // 6. Update status to 'integrating'
    await supabase
      .from('pdf_ingestions')
      .update({
        status: 'integrating',
    ...
    // 7. Load lookup data for matching
    const [facultyRes, coursesRes, roomsRes, sectionsRes] = await Promise.all([
      ...
      supabase.from('courses').select('id, code, semester, branch').eq('department_id', ingestion.department_id).is('deleted_at', null),
      supabase.from('rooms').select('id, room_number').eq('department_id', ingestion.department_id).is('deleted_at', null),
      supabase.from('sections').select('id, section_label, branch, semester').eq('department_id', ingestion.department_id),
    ]);
    ...
    for (let i = 0; i < parseResult.rows.length; i++) {
      ...
      try {
        ...
        if (!course) {
          ...
          console.log(`Course not found: ${row.courseCode}. Auto-creating for Sem ${inferredSem}, Branch ${inferredBr}`);
        ...
        if (row.roomNumber && row.roomNumber.trim()) {
          ...
          if (!room) {
            ...
            if (roomCreateError || !newRoom) {
              ...
              // Don't fail — continue with null room
            } else {
        ...
        if (row.facultyName && row.facultyName.trim()) {
          ...
          if (match) {
            ...
            if (match.score < 1) {
              console.log(`Faculty fuzzy match: "${row.facultyName}" -> "${match.match}" (score: ${match.score.toFixed(2)}, method: ${match.method})`);
            }
          } else {
            errorLog.push({
              row: i + 1,
        ...
        if (!section) {
          console.log(`Section not found for Branch: ${resolvedCourse.branch}, Sem: ${resolvedCourse.semester}, Label: ${sectionLabel}. Auto-creating...`);
    ...
    console.log(`[Ingestion ${ingestionId}] Completed processing with status: ${finalStatus}. Created: ${createdCount}, Matched: ${matchedCount}, Failed: ${failedCount}`);
    ...
    console.error('Fatal error in PDF processing route:', error);
